...
def _ensure_vader_lexicon() -> None:
    ...
    Runs lazily (only when labeling is actually performed) rather than at
    import time, so importing this module never triggers a network call.
    Failures are non-fatal: if the lexicon is already cached locally the
    download is a no-op, and if the network is unavailable we let
    ``SentimentIntensityAnalyzer()`` raise a clear error at use time.
    """
    try:
        nltk.data.find("sentiment/vader_lexicon.txt")
    except LookupError:
...
def label_data(
    df: pd.DataFrame, positive_threshold: float, negative_threshold: float
) -> pd.DataFrame:
    ...
    try:
        ...
        df["category"] = df["sentiment_polarity"].apply(
            lambda score: (
                1 if score >= positive_threshold else (-1 if score <= negative_threshold else 0)
            )
        )
        logger.debug(
            "VADER labeling completed. Distribution: %s", df["category"].value_counts().to_dict()
        )
        return df

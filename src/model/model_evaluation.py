...
def get_tracking_uri() -> str:
    """Resolve the MLflow tracking URI (env var override, local sqlite default)."""
    return os.environ.get("MLFLOW_TRACKING_URI") or "sqlite:///mlflow.db"
...
def evaluate_model(model: Any, X_test: np.ndarray, y_test: np.ndarray) -> tuple[dict, np.ndarray]:
...
def _flatten_params(params: dict, prefix: str = "") -> dict:
    """Flatten nested params dict so each value is a scalar MLflow can log."""
    flat = {}
...
def main() -> None:
    ...
    with mlflow.start_run() as run:
        try:
            ...
            # Log parameters (flatten nested YAML to scalar key/value pairs)
            ...
            mlflow.set_tag("dataset", "Squid Game Tweets")
